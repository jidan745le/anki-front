...
// 不带语音功能，稳定版（修复eventsouce error，有一些bug）
      ...
      pendingMessages.forEach((pendingMessage, index) => {
        ...
        statusEventSource.onmessage = event => {
          ...
          if (jsonData.event === 'existing_content') {
            ...
            // Handle complete event - backend sends complete events through regular message events
            try {
              // Update message with complete content and remove pending state
              updateMessageContent(sessionId, jsonData.content, false);
    ...
    if (useStreaming) {
      ...
      try {
        ...
        eventSource.onmessage = event => {
          const eventData = event.data;
          const jsonData = JSON.parse(eventData);
          console.log('message111', jsonData);
          ...
          if (jsonData.event === 'message') {
            ...
            // 处理完成事件 - 后端通过普通message发送complete事件
        ...
        eventSource.onerror = error => {
          ...
          if (readyState === 2) {
            ...
            // 检查是否是会话过期的情况
            const url =
              eventSourceTarget && 'url' in eventSourceTarget ? eventSourceTarget.url : undefined;
            if (url && typeof url === 'string' && url.includes('/aichat/stream/')) {
              console.log('可能是会话过期导致的连接关闭，这是正常现象');
            }
