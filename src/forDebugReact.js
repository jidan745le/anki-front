...
const TestList = () => {
  ...
  return <><ul onClick={initList} className={styles.ulContainer} >
    <button>第一次更新点我，生成5个列表项，比初始多2项</button>
    {list.map((item, index) => <li key={index} className={styles.ulContainerLi}>
      {item}
      <input></input>
      <a style={{ marginLeft: "2px" }} onClick={(e) => {
        e.stopPropagation()
        list.splice(index, 1)
        setList([...list])
      }}>delete</a>
...
function AnkiCard({ flipped, onFlip, onNext, front, back }) {
  return <div>
    ...
    {flipped ? <div><MyEditor value={`<div>${back}</div>`} /></div> : <div style={{ height: "90vh" }}></div>}
    {<div style={{ display: "flex", justifyContent: "center" }}>
      {flipped ? <Button onClick={() => { onNext && onNext() }}>通过</Button> :
        <Button onClick={() => { onFlip && onFlip(true) }}>展示答案</Button>}
    </div>}
  </div>
}
